...
<script>
  ...
  function cachePopperAndTooltip(id, chord) {
    ...
    if (!(id in poppers)) {
      ...
      const tooltip = tooltips[chord];

      const popperInstance = Popper.createPopper(ref, tooltip, {
        placement: 'top',
        modifiers: [
          {
            name: 'offset',
      ...
      poppers[id] = popperInstance;
    }
  }

  function showPopper(id, chord) {
    cancelHidePopper(chord);
    immediatelyHideAllPoppers(chord);
    cachePopperAndTooltip(id, chord);
    tooltips[chord].setAttribute('data-show', '');
    poppers[id].update();
  }

  function immediatelyHideAllPoppers(exceptChord) {
    for (const chord in tooltips) {
      if (chord != exceptChord) {
        immediatelyHidePopper(chord);
      }
    }
  }

  function immediatelyHidePopper(chord) {
    tooltips[chord].removeAttribute('data-show');
  }

  function hidePopper(chord) {
  ...
  function cancelHidePopper(chord) {
