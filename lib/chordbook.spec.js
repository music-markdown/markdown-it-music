...
describe('Guitar Chordbook', () => {
  test.each([
    ['F', 1], ['F#', 2], ['Gb', 2], ['G', 3], ['G#', 4], ['Ab', 4], ['A', 5],
    ['A#', 6], ['Bb', 6], ['B', 7], ['C', 8], ['C#', 9], ['Db', 9], ['D', 10],
    ['D#', 11], ['Eb', 11], ['E', 12], ['F', 13], ['F#', 14], ['Gb', 14],
    ['G', 15], ['G#', 16], ['Ab', 16],
  ])('should contain %s barre chord as E shaped chord offset by %s', (name, os) => {
    expect(guitarChordbook.get(name))
      .toContainEqual(parseVoicing(`o${os} b1,6,${os} n2,${2+os} n3,${2+os} n4,${1+os}`));
  });

  test.each([
    ['A#', 1], ['Bb', 1], ['B', 2], ['C', 3], ['C#', 4], ['Db', 4], ['D', 5],
    ['D#', 6], ['Eb', 6], ['E', 7], ['F', 8], ['F#', 9], ['Gb', 9], ['G', 10],
    ['G#', 11], ['Ab', 11], ['A', 12], ['A#', 13], ['Bb', 13], ['B', 14],
    ['C', 15], ['C#', 16], ['Db', 16],
  ])('should contain %s barre chord as A shaped chord offset by %s', (name, os) => {
    expect(guitarChordbook.get(name))
      .toContainEqual(parseVoicing(`o${os} b1,6,${os} n3,${2+os} n4,${2+os} n5,${2+os}`));
  });

  test.each([
    ['G#', 1], ['Ab', 1], ['A', 2], ['A#', 3], ['Bb', 3], ['B', 4], ['C', 5],
    ['C#', 6], ['Db', 6], ['D', 7], ['D#', 8], ['Eb', 8], ['E', 9], ['F', 10],
    ['F#', 11], ['Gb', 11], ['G', 12], ['G#', 13], ['Ab', 13], ['A', 14],
    ['A#', 15], ['Bb', 15], ['B', 16],
  ])('should contain %s barre chord as G shaped chord offset by %s', (name, os) => {
    expect(guitarChordbook.get(name))
      .toContainEqual(parseVoicing(`o${os} b3,5,${os} n1,${3+os} n2,${2+os} n6,${3+os}`));
  });
