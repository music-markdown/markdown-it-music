...
const compareVoicings = (voicing1, voicing2) => {
  ...
  return (voicing1.mutes.length + voicing1.notes.length) - (voicing2.mutes.length + voicing2.notes.length);
};
...
function parseVoicing(str) {
  ...
  for (const rule of str.split(/\s/)) {
    ...
    if (ruleType == 'o') {
      ...
      voicing.barres.push({ first: params[0], last: params[1], fret: params[2] });
    } else {
