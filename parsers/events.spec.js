import { Line, convertVerseToEvents } from "./events";
...
describe("Event", () => {
  test("should add voices", () => {
    ...
    const nextVoices = new Map([
      ...
      ["l1", [{ index: 4, content: "longest" }]],
      ["a1", [{ index: 4, content: "crash!" }]],
    ]);

    const line = new Line();
    const actualLine = line.createLineFromPhrase(nextVoices, [
      "c1",
      "c2",
      "l1",
      "a1",
  ...
  test("should only add voices that are in index range", () => {
    ...
    const line = new Line();
    const actualLine = line.createLineFromPhrase(phrase, [
      "c1",
      "c2",
      "l1",
      "l2",
  ...
  test("should add next index first", () => {
    const phrase = new Map([
      ["c1", [{ index: 0, content: "G" }]],
      ["c2", [{ index: 40, content: "C" }]],
      ["l1", [{ index: 0, content: "The" }]],
      ["l2", [{ index: 40, content: "Test" }]],
    ]);
  ...
  test("should include offset when a voice is not at the start index of an event", () => {
    const phrase = new Map([
      ["c1", [{ index: 2, content: new Chord("C") }]],
      ["l1", [{ index: 0, content: "Testing" }]],
    ]);
  ...
  test("should not add a voice if the next event would occur before the voice", () => {
    const verse = [
      new Map([
        ["c1", [{ index: 19, content: new Chord("Am") }]],
        [
          "v1",
          [
            { index: 0, content: "All" },
