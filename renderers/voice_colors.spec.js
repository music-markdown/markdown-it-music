...
describe("Voice Colors", () => {
  ...
  test("should be consistent in color generated order so when you come back, it is the same color", () => {
    ...
    expect(firstVoiceColors.getVoiceColor("c1")).toEqual(
      secondVoiceColors.getVoiceColor("c1")
    );
    expect(firstVoiceColors.getVoiceColor("l1")).toEqual(
      secondVoiceColors.getVoiceColor("l1")
    );
  });
});
